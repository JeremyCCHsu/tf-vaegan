...
try:
	for step in range(n_step):
		...
		if step % step_per_ep == 0:	
			creation = sess.run(machine.decode())
			real = sess.run(imgs)

			plt.figure()
			plt.subplot(211)
			plt.imshow(creation[0, :, :, 0], interpolation=None, cmap='gray')
			plt.colorbar()
			plt.subplot(212)
			plt.imshow(real[0, :, :, 0], interpolation=None, cmap='gray')
			plt.colorbar()
			plt.savefig('test-{:4d}.png'.format(step))
